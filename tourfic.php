 ...
 * Description:     The ultimate WordPress tour management plugin for hotel booking, tour operator and travel agency
 * websites. Manage all your online Travel Booking system along with order system and any payment of WooCommerce.
 * Author:          Themefic Author URI:      https://themefic.com Text Domain:     tourfic Domain Path:     /lang/
 * Version:         2.1.1 Tested up to: 5.9.2 WC tested up to: 6.3.1 Requires PHP: 7.1
 */
...
const TOURFIC_TEMPLATES_URL = TOURFIC_PLUGIN_URL . 'templates/';
const TOURFIC_ADMIN_URL     = TOURFIC_PLUGIN_URL . 'admin/';
const TF_TEMPLATES_URL      = TF_URL . 'templates/';
const TF_ADMIN_URL          = TF_URL . 'admin/';
const TF_ASSETS_URL         = TF_URL . 'assets/';
// Paths
const TF_ADMIN_PATH         = TF_PATH . 'admin/';
const TF_INC_PATH           = TF_PATH . 'inc/';
const TF_TEMPLATE_PATH      = TF_PATH . 'templates/';
const TF_TEMPLATE_PART_PATH = TF_TEMPLATE_PATH . 'template-parts/';
const TF_OPTIONS_PATH       = TF_ADMIN_PATH . 'options/';
const TF_ASSETS_PATH        = TF_PATH . 'assets/';
const TFD                   = 'tourfic';
...
if (!function_exists('tf_enqueue_main_admin_scripts')) {
    function tf_enqueue_main_admin_scripts()
    {
        ...
        wp_enqueue_script('tf', TF_ADMIN_URL . 'assets/js/admin.js', ['jquery'], '2.1.0', true);
        wp_localize_script(
            'tf',
            'tf_params',
            [
...
function tf_file_missing($files = '')
{
    ...
    if (is_admin()) {
        if (!empty($files)) {
            ...
            $message = '<strong>' . $files . '</strong>' . __(' file is missing! It is required to function Tourfic properly!', TFD);
...
?>
            ...
            <div id="message" class="error">
                <p><?php printf(__('Tourfic requires %1$s WooCommerce %2$s to be activated.', TFD), '<strong><a href="https://wordpress.org/plugins/woocommerce/" target="_blank">', '</a></strong>'); ?></p>
                <p><a class="install-now button" href="<?php echo esc_url(admin_url('/plugin-install.php?s=slug:woocommerce&tab=search&type=term')); ?>"><?php esc_attr_e('Install Now', TFD); ?></a>
                </p>
            </div>
        ...
        ?>
            ...
            <div id="message" class="error">
                <p><?php printf(__('Tourfic requires %1$s WooCommerce %2$s to be activated.', TFD), '<strong><a href="https://wordpress.org/plugins/woocommerce/" target="_blank">', '</a></strong>'); ?></p>
                <p>
                    <a href="<?php echo get_admin_url(); ?>plugins.php?_wpnonce=<?php echo wp_create_nonce('activate-plugin_woocommerce/woocommerce.php'); ?>&action=activate&plugin=woocommerce/woocommerce.php" class="button activate-now button-primary"><?php esc_attr_e('Activate', TFD); ?></a>
                </p>
            </div>
        ...
        ?>
            ...
            <div id="message" class="error">
                <p><?php printf(__('%sTourfic is inactive.%s This plugin requires WooCommerce 2.5 or newer. Please %supdate WooCommerce to version 2.5 or newer%s', TFD), '<strong>', '</strong>', '<a href="' . admin_url('plugins.php') . '">', '&nbsp;&raquo;</a>'); ?></p>
            </div>
