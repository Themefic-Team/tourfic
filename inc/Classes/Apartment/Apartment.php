...
?>
	...
	public static function tf_apartment_search_form_horizontal($classes, $title, $subtitle, $advanced, $design)
	{
		...
		<?php } elseif (!empty($design) && 4 == $design) { ?>
			<form class="tf-archive-search-box-wrapper tf_apt-shortcode-design-4 tf-shortcode-design-4 <?php echo esc_attr($classes); ?>" id="tf_apartment_booking" method="get" autocomplete="off" action="<?php echo esc_url(Helper::tf_booking_search_action()); ?>">
				<div class="tf-date-selection-form">
					<div class="tf-date-select-box tf-flex tf-flex-gap-8">
						<div class="tf-date-single-select tf-flex tf-flex-gap-8 tf-flex-space-bttn tf-pick-drop-location full-width">
							<div class="tf-select-date">
				...
				$(document).ready(function() {
					...
					const checkinoutdateange = flatpickr("#tf-apartment-booking #check-in-out-date", {
						...
						<?php if (!empty($checked_enable_dates) && is_array($checked_enable_dates)) : ?>
							enable: [<?php array_walk($checked_enable_dates, function ($date) {
											echo '"' . esc_html($date) . '",';
										}); ?>],
						<?php endif; ?>
						disable: [
						...
						<div class="tf-card-heading-info">
							<div class="tf-section-title-and-location">
								<a href="<?php echo esc_url(get_the_permalink($post_id)); ?>">
									<h2 class="tf-section-title"><?php echo esc_html(Helper::tourfic_character_limit_callback(get_the_title($post_id), 55)); ?></h2>
								</a>
								<?php
								if (! empty($address)) {
								?>
						...
						<div class="tf_property_block_main_row">
							<div class="tf_item_main_block">
								<div class="tf-hotel__title-wrap">
									<a href="<?php echo esc_url($url); ?>">
										<h3 class="tourfic_hotel-title"><?php echo esc_html(get_the_title($post_id)); ?></h3>
									</a>
								</div>
								<?php
								if (!empty($address)) {
