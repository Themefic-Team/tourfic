...
class Helper {
	...
	/**
	 * Safely decode a meta value that may be a JSON string or already a PHP array.
	 * Prevents json_decode() TypeError on PHP 8.x when given an array.
	 *
	 * @param mixed $value   The meta value (string or array).
	 * @param mixed $default Default to return if empty (default: []).
	 * @return array
	 */
	static function tf_json_to_array( $value, $default = [] ) {
		if ( empty( $value ) ) {
			return $default;
		}
		if ( is_array( $value ) ) {
