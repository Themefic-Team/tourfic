...
use Tourfic\Classes\Room\Availability as Room_Availability;
...
class Hotel {
	...
	public function __construct() {
		...
		\Tourfic\App\Hotel_Booking_Popup::instance();
	...
	private static function get_requested_room_count( $room ) {
		return max( 1, intval( $room ) );
	}

	private static function get_capacity_values_by_room_units( array $rooms_meta, $capacity_key ) {
	...
	private static function get_total_room_units( array $rooms_meta ) {
		$total_room_units = 0;
	...
	private static function room_unit_capacity_passes( array $capacity_values, $requested_people, $requested_rooms ) {
		...
		return array_sum( array_slice( $capacity_values, 0, $requested_rooms ) ) >= intval( $requested_people );
	}

	private static function get_period_date_strings( $period ) {
		$date_strings = array();
	...
	private static function filter_room_metas_available_for_period( array $rooms_meta, array $date_strings ) {
		...
		return array_filter( $rooms_meta, function( $room_meta ) use ( $date_strings ) {
			...
			return Room_Availability::are_dates_available_for_rules( $room_meta['avail_date'], $date_strings );
		} );
	}

	private static function get_room_check_in_price_data( array $room_meta, $check_in_date ) {
		$price_data = array();
		$rules      = Room_Availability::normalize_availability_rules( $room_meta['avail_date'] ?? '' );

		foreach ( $rules as $rule ) {
			$date_status = ! empty( $rule['status'] ) ? $rule['status'] : 'available';
			if ( 'unavailable' === $date_status || empty( $rule['check_in'] ) || $check_in_date !== $rule['check_in'] ) {
				continue;
			}

			$price_data['price']       = $rule['price'] ?? '';
			$price_data['adult_price'] = $rule['adult_price'] ?? '';
			$price_data['child_price'] = $rule['child_price'] ?? '';

			$options_count = $rule['options_count'] ?? 0;
			for ( $i = 0; $i <= $options_count - 1; $i ++ ) {
				$option_pricing_type = ! empty( $rule[ 'tf_option_pricing_type_' . $i ] ) ? $rule[ 'tf_option_pricing_type_' . $i ] : '';
				if ( ! empty( $rule[ 'tf_room_option_' . $i ] ) && 'per_room' === $option_pricing_type ) {
					$price_data[ 'tf_option_room_price_' . $i ] = ! empty( $rule[ 'tf_option_room_price_' . $i ] ) ? $rule[ 'tf_option_room_price_' . $i ] : 0;
				} elseif ( ! empty( $rule[ 'tf_room_option_' . $i ] ) && 'per_person' === $option_pricing_type ) {
					$price_data[ 'tf_option_adult_price_' . $i ] = ! empty( $rule[ 'tf_option_adult_price_' . $i ] ) ? $rule[ 'tf_option_adult_price_' . $i ] : 0;
					$price_data[ 'tf_option_child_price_' . $i ] = ! empty( $rule[ 'tf_option_child_price_' . $i ] ) ? $rule[ 'tf_option_child_price_' . $i ] : 0;
				}
			}
	...
	/**
	 * Get normalized hotel meta for mixed schema support.
	 *
	 * @param int  $hotel_id     Hotel post ID.
	 * @param bool $sync_legacy  Persist fallback legacy keys when true.
	 *
	 * @return array
	 */
	public static function get_normalized_hotel_meta( $hotel_id, $sync_legacy = false ) {

		$hotel_meta = get_post_meta( $hotel_id, 'tf_hotels_opt', true );
		$hotel_meta = is_array( $hotel_meta ) ? $hotel_meta : array();
		$changed    = false;

		$map_data = self::get_hotel_map_data( $hotel_meta );
		if ( ! empty( $map_data ) && empty( $hotel_meta['map'] ) ) {
			$hotel_meta['map'] = $map_data;
			$changed           = true;
		}

		$missing_featured_key = ! isset( $hotel_meta['featured'] ) || '' === $hotel_meta['featured'];
		if ( $missing_featured_key && isset( $hotel_meta['hotel_as_featured'] ) ) {
			$hotel_meta['featured'] = $hotel_meta['hotel_as_featured'];
			$changed                = true;
		}

		if ( $sync_legacy && $changed ) {
	...
	/**
	 * Check featured status from hotel meta with mixed schema support.
	 *
	 * @param array $hotel_meta Hotel meta array.
	 *
	 * @return bool
	 */
	public static function is_featured_hotel_meta( $hotel_meta ) {

		if ( ! is_array( $hotel_meta ) ) {
			return false;
		}
		...
		return ! empty( $hotel_meta['hotel_as_featured'] );
	}

	/**
	 * Get map data from hotel meta with location fallback support.
	 *
	 * @param array $hotel_meta Hotel meta array.
	 *
	 * @return array
	 */
	public static function get_hotel_map_data( $hotel_meta ) {
		...
		if ( ! empty( $hotel_meta['map'] ) ) {
			$map_data = Helper::tf_data_types( $hotel_meta['map'] );
		}

		if ( empty( $map_data ) && ! empty( $hotel_meta['location'] ) ) {
			$map_data = Helper::tf_data_types( $hotel_meta['location'] );
		}

		if ( is_object( $map_data ) ) {
			$map_data = (array) $map_data;
		}

		return is_array( $map_data ) ? $map_data : array();
	}

	/**
	 * Get hotel archive features with facilities fallback for new schema.
	 *
	 ...
	 * @return array
	 */
	private static function get_archive_feature_items( $post_id, $hotel_meta ) {
		...
		$seen_features     = array();
		$facilities        = ! empty( $hotel_meta['facilities'] ) && is_array( $hotel_meta['facilities'] ) ? $hotel_meta['facilities'] : array();
		...
		foreach ( $facilities as $facility ) {
			...
			if ( is_numeric( $facility ) ) {
				$facility_term = get_term( absint( $facility ), 'hotel_feature' );
			} else {
				$facility_slug = sanitize_title( wp_strip_all_tags( (string) $facility ) );
				if ( ! empty( $facility_slug ) ) {
					$facility_term = get_term_by( 'slug', $facility_slug, 'hotel_feature' );
				}
				$feature_name = ucwords( str_replace( array( '-', '_' ), ' ', sanitize_text_field( (string) $facility ) ) );
			}

			if ( $facility_term && ! is_wp_error( $facility_term ) ) {
				$feature_name = $facility_term->name;
			}
			...
			$feature_key = sanitize_title( $feature_name );
			if ( isset( $seen_features[ $feature_key ] ) ) {
				continue;
			}
			$seen_features[ $feature_key ] = true;

			if ( $facility_term && ! is_wp_error( $facility_term ) ) {
				$fallback_features[] = $facility_term;
			} else {
				$fallback_features[] = (object) array(
					'name'             => $feature_name,
					'term_taxonomy_id' => 0,
				);
			}
	...
	function tf_room_availability_callback() {
        ...
        $requested_rooms   = self::get_requested_room_count( $form_room );
        ...
        $first_location_name = ! empty( $locations ) ? $locations[0]->name : '';
	        $show_room_service_popup = apply_filters( 'tourfic_hotel_room_service_popup_visibility', true, $hotel_id, $meta );
	...
	static function tf_filter_hotel_by_date( $period, array &$not_found, array $data = [] ): void {
		...
		$requested_rooms         = self::get_requested_room_count( $room );
		$searching_period        = self::get_period_date_strings( $period );
		$capacity_rooms_meta     = self::filter_room_metas_available_for_period( $rooms_meta, $searching_period );
		$adult_capacities        = self::get_capacity_values_by_room_units( $capacity_rooms_meta, 'adult' );
		$adult_capacity_pass     = self::room_unit_capacity_passes( $adult_capacities, $adults, $requested_rooms );
		$adult_result = ! empty( $adult_capacities );

		$child_capacities = self::get_capacity_values_by_room_units( $capacity_rooms_meta, 'child' );
		$child_capacity_pass = self::room_unit_capacity_passes( $child_capacities, $child, $requested_rooms );
		$childs_result = ! empty( $child_capacities );

		$total_room_units = self::get_total_room_units( $capacity_rooms_meta );
		...
		if ( $adult_result && $adult_capacity_pass && $childs_result && $child_capacity_pass && $room_validation ) {
			...
			if ( in_array( 0, $avil_by_date ) || empty( $avil_by_date ) || empty( $avil_by_date[0] ) ) {
				...
				if ( ! empty( $rooms ) && ! empty( $startprice ) && ! empty( $endprice ) ) {
					...
					foreach ( $rooms as $_room ) {
						if ( empty( $capacity_rooms_meta[ $_room->ID ] ) ) {
							continue;
						}

						$room = get_post_meta( $_room->ID, 'tf_room_opt', true );
						if ( ! empty( $startprice ) && ! empty( $endprice ) ) {
							$tf_check_in_date_price = self::get_room_check_in_price_data( $room, $tf_check_in_date );
	...
	static function tf_filter_hotel_without_date( $period, array &$not_found, array $data = [] ): void {
		...
		$requested_rooms     = self::get_requested_room_count( $room );
		$adult_capacities    = self::get_capacity_values_by_room_units( $rooms_meta, 'adult' );
		$adult_capacity_pass = self::room_unit_capacity_passes( $adult_capacities, $adults, $requested_rooms );
		$adult_result = ! empty( $adult_capacities );

		$child_capacities = self::get_capacity_values_by_room_units( $rooms_meta, 'child' );
		$child_capacity_pass = self::room_unit_capacity_passes( $child_capacities, $child, $requested_rooms );
		$childs_result = ! empty( $child_capacities );

		$total_room_units = self::get_total_room_units( $rooms_meta );
		$room_validation = $total_room_units >= $requested_rooms;
		...
		if ( $tf_hotel_selected_template == "design-1" ) { ?>
            <form id="tf-single-hotel-avail" class="widget tf-hotel-booking-sidebar tf-booking-form" method="get" autocomplete="off">
                ...
                <div class="tf_form-row">
                    ...
                    <input type="hidden" name="room" value="<?php echo esc_attr( $room_selected ); ?>"/>
	...
	static function hotel_booking_popup( $post_id, $room_id, $adult, $child ) {
		...
		$booking_confirmation_extension = apply_filters(
			'tourfic_hotel_booking_confirmation_extension',
			array(),
			$post_id,
			$meta,
			3
		);
		$has_booking_confirmation_extension = ! empty( $booking_confirmation_extension['enabled'] );
		$booking_confirmation_step = ! empty( $booking_confirmation_extension['step'] )
			? absint( $booking_confirmation_extension['step'] )
			: 3;
		$booking_confirmation_label = ! empty( $booking_confirmation_extension['label'] )
			? sanitize_text_field( $booking_confirmation_extension['label'] )
			: esc_html__( 'Booking Confirmation', 'tourfic' );
		$booking_confirmation_icon = ! empty( $booking_confirmation_extension['icon'] )
			? sanitize_html_class( $booking_confirmation_extension['icon'] )
		...
		<?php do_action( 'tourfic_hotel_booking_confirmation_notice', $post_id, $meta ); ?>
        <div class="tf-withoutpayment-booking tf-hotel-withoutpayment-booking">
            <div class="tf-withoutpayment-popup">
                        ...
                        <ul>
							...
							if ( $airport_service_type || $has_booking_extension ) {
                                ...
                                <li class="tf-booking-step tf-booking-step-1 active">
                                    <i class="ri-price-tag-3-line"></i> <?php echo esc_html__( "Hotel Service", "tourfic" ); ?>
                                </li>
							<?php }
							...
							<?php }
							if ( $has_booking_confirmation_extension ) {
								$active_steps[ $booking_confirmation_step ] = $booking_confirmation_step;
								?>
                                <li class="tf-booking-step tf-booking-step-<?php echo esc_attr( $booking_confirmation_step ); ?> <?php echo empty( $airport_service_type ) && ! $has_booking_extension && empty( $enable_guest_info ) ? esc_attr( 'active' ) : ''; ?>">
                                    <i class="<?php echo esc_attr( $booking_confirmation_icon ); ?>"></i> <?php echo esc_html( $booking_confirmation_label ); ?>
                                </li>
							<?php } ?>
                        </ul>
                ...
                <div class="tf-booking-content-summery">
					...
					<?php }
					do_action(
						'tourfic_hotel_render_booking_confirmation_extension',
						$post_id,
						$meta,
						$booking_confirmation_extension,
						array(
							'has_booking_extension' => $has_booking_extension,
							'airport_service'       => ! empty( $airport_service_type ),
							'guest_info'            => ! empty( $enable_guest_info ),
							'step'                  => $booking_confirmation_step,
							'heading'               => $hotel_guest_details_text,
						)
					);
					?>

                    <!-- Popup Booking Summery -->
                    <div class="tf-booking-summery" style="<?php echo empty( $airport_service_type ) && ! $has_booking_extension && empty( $enable_guest_info ) && ! $has_booking_confirmation_extension ? esc_attr( "width: 100%;" ) : ''; ?>">
                ...
                <div class="tf-booking-pagination tf-hotel-booking-pagination">
					<?php if ( empty( $airport_service_type ) && ! $has_booking_extension && empty( $enable_guest_info ) && ! $has_booking_confirmation_extension ) { ?>
					...
					if ( $airport_service_type || $has_booking_extension ) { ?>
                        <div class="tf-control-pagination show tf-pagination-content-1">
							<?php if ( empty( $enable_guest_info ) && ! $has_booking_confirmation_extension ) { ?>
							...
							<?php } else { ?>
                                <a href="#" class="tf-next-control tf-tabs-control tf_btn" data-step="1"><?php echo ! empty( Helper::tfopt( 'hotel_service_popup_action' ) ) ? esc_html( Helper::tfopt( 'hotel_service_popup_action' ) ) : esc_html__( "Continue", "tourfic" ); ?></a>
							<?php } ?>
                        </div>
					<?php }
					if ( $enable_guest_info ) { ?>
                        ...
                        <div class="tf-control-pagination tf-pagination-content-2 <?php echo empty( $airport_service_type ) && ! $has_booking_extension ? esc_attr( 'show' ) : ''; ?>">
							<?php if ( $airport_service_type || $has_booking_extension ) { ?>
							...
							<?php } else { ?>
                                <button type="submit" class="tf-traveller-error tf_btn hotel-room-book"><?php echo esc_html__( "Continue", "tourfic" ); ?></button>
							<?php } ?>
                        </div>
					<?php }
					if ( $has_booking_confirmation_extension ) { ?>
                        ...
                        <div class="tf-control-pagination tf-pagination-content-<?php echo esc_attr( $booking_confirmation_step ); ?> <?php echo empty( $airport_service_type ) && ! $has_booking_extension && empty( $enable_guest_info ) ? esc_attr( 'show' ) : ''; ?>">
							<?php if ( $airport_service_type || $has_booking_extension || $enable_guest_info ) { ?>
                                <a href="#" class="tf-back-control tf-step-back" data-step="<?php echo esc_attr( $booking_confirmation_step ); ?>"><i class="fa fa-angle-left"></i><?php echo esc_html__( "Back", "tourfic" ); ?></a>
							<?php } ?>
		...
		if ( ! empty( $meta['airport_service'] ) && 1 == $meta['airport_service'] ) {
			if ( "pickup" == $airport_service ) {
				...
				if ( "per_person" == $airport_pickup_price['airport_pickup_price_type'] ) {
					$service_adult_fee           = ! empty( $airport_pickup_price['airport_service_fee_adult'] ) ? $airport_pickup_price['airport_service_fee_adult'] : 0;
					$service_child_fee           = ! empty( $airport_pickup_price['airport_service_fee_children'] ) ? $airport_pickup_price['airport_service_fee_children'] : 0;
					$airport_service_price_total = ( (int) $adult * $service_adult_fee ) + ( (int) $child * $service_child_fee );
	 ...
	 */
	static function tf_hotel_archive_single_item( $adults = '', $child = '', $room = '', $check_in_out = '', $startprice = '', $endprice = '', $settings = [] ) {

		// get post id
		$post_id = get_the_ID();
		$meta    = self::get_normalized_hotel_meta( $post_id );
		$features = self::get_archive_feature_items( $post_id, $meta );

		// Location
		$address = '';
		$map     = self::get_hotel_map_data( $meta );
                        ...
                        <div class="tf-archive-features tf-mt-16">
                            <ul>
								<?php foreach ( $features as $tfkey => $feature ) {
									...
									$feature_id   = ! empty( $feature->term_taxonomy_id ) ? absint( $feature->term_taxonomy_id ) : 0;
									if ( ! empty( $feature_id ) ) {
									...
									if ( $tfkey < $features_count && ! empty( $feature_name ) ) {
										?>
	...
	function tf_hotel_rooms_assign( $post_id, $post, $old_status ) {
		...
		$removed_rooms = (is_array($rooms) && is_array($assigned_room_ids)) ? array_diff($assigned_room_ids, $rooms) : [];
	...
	static function tf_hotel_order_inventory_data( $order_id ) {
		$order_id = absint( $order_id );
		if ( empty( $order_id ) ) {
			return;
		}
