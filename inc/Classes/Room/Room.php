...
class Room {
	...
	static function tf_filter_room_by_date( $period, array &$not_found, array $data = [] ): void {
		...
		if ( ( empty( $adults ) || ( ! empty( $room_meta['adult'] ) && $room_meta['adult'] >= $adults_per_room ) ) && ( empty( $child ) || ( ! empty( $room_meta['child'] ) && $room_meta['child'] >= $childs_per_room ) ) && ! empty( $room_meta['num-room'] ) && $room_meta['num-room'] >= $requested_rooms ) {
		...
		if ( ( empty( $adults ) || ( ! empty( $room_meta['adult'] ) && $room_meta['adult'] >= $adults_per_room ) ) && empty( $room_meta['child'] ) && $room_meta['child'] >= $child && ! empty( $room_meta['num-room'] ) && $room_meta['num-room'] >= $requested_rooms ) {
	...
	static function tf_room_sidebar_booking_form( $b_check_in = '', $b_check_out = '', $design = '' ) {
		...
		$room_option = ! empty( $_GET['room-option'] ) ? sanitize_text_field( $_GET['room-option'] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		// Check-in & out date
		$check_in_out = ! empty( $_GET['check-in-out-date'] ) ? sanitize_text_field( $_GET['check-in-out-date'] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
