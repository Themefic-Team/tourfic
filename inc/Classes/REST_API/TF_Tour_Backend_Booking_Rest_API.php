...
if ( ! class_exists( 'TF_Tour_Backend_Booking_Rest_API' ) ) {
	class TF_Tour_Backend_Booking_Rest_API extends TF_Rest_API {
		...
		/*
		 * Tour available date time
		 */
		public function tf_available_tour_date_time( $request ) {
			$departure_date = $return_date = $min_people = $max_people = '';
			...
			$tf_tour_global_template = ! empty( Helper::tf_data_types( tfopt( 'tf-template' ) )['single-tour'] ) ? Helper::tf_data_types( tfopt( 'tf-template' ) )['single-tour'] : 'design-1';
		 ...
		 * Tour Add booking
		 */
		public function tf_tour_add_booking( $request ) {
			$response = array(
				'success' => false,
			);

			$selected_package = isset( $request['tf_tour_packages'] ) && '' !== $request['tf_tour_packages'] ? absint( $request['tf_tour_packages'] ) : '';
			if ( '' === (string) $selected_package ) {
				$selected_package = $this->tf_get_default_tour_package_id( intval( $request['tf_available_tours'] ) );
			}
			$res              = $this->tf_get_tour_total_price( intval( $request['tf_available_tours'] ), $request['tf_tour_date'], strval($request['tf_tour_time']), $request['tf_tour_extras'], intval($request['tf_tour_adults_number']), intval($request['tf_tour_children_number']), intval($request['tf_tour_infants_number']), $selected_package );
			...
			if ( $request['tf_tour_date'] ) {
				list( $tour_in, $tour_out ) = explode( ' - ', $request['tf_tour_date'] );
			}

			$tf_package_title = '';
			if ( '' !== (string) $selected_package ) {
				$selected_package_data = $this->tf_get_tour_package_title( intval( $request['tf_available_tours'] ), $selected_package );
				$tf_package_title      = ! empty( $selected_package_data['tf_tour_package_title'] ) ? $selected_package_data['tf_tour_package_title'] : '';
			}

			$order_details = [
				'order_by'    => $request['tf_tour_booked_by'],
				'tour_date'   => $res['tour_date'],
				'tour_time'   => $res['tf_tour_time_title'],
				'tour_extra'  => $res['tf_tour_extra_title'],
				'package'     => $tf_package_title,
				'adult'       => $request['tf_tour_adults_number'],
				'child'       => $request['tf_tour_children_number'],
				'infants'     => $request['tf_tour_infants_number'],
				'total_price' => $res['tf_tour_price'],
				'due_price'   => '',
			];

			$order_data = array(
				'post_id'          => intval( $request['tf_available_tours'] ),
				'post_type'        => 'tour',
				'room_number'      => null,
				'check_in'         => $tour_in,
				'check_out'        => $tour_out,
...
TF_Tour_Backend_Booking_Rest_API::get_instance();
