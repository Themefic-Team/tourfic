...
class Enqueue {
	...
	public function __construct() {
		$this->css_min = ! empty( Helper::tfopt( 'css_min' ) ) ? '.min' : '';
		$this->js_min  = ! empty( Helper::tfopt( 'js_min' ) ) ? '.min' : '';
	...
	function tf_enqueue_scripts() {
		 ...
		 * Custom
		 */
		wp_enqueue_script( 'tourfic', TF_ASSETS_APP_URL . 'js/tourfic-scripts' . $this->js_min . '.js', array('wp-hooks'), TF_VERSION, true );
		wp_localize_script( 'tourfic', 'tf_params',
			array(
				...
				'adult'                  => apply_filters( 'tf_tour_adult_label', esc_html__( 'Adult', 'tourfic' ), true, true ),
				'children'               => apply_filters( 'tf_tour_child_label', esc_html__( 'Children', 'tourfic' ), true, true ),
				'infant'                 => apply_filters( 'tf_tour_infant_label', esc_html__( 'Infant', 'tourfic' ), true, true ),
	...
	*/
	...
	public function tf_options_admin_enqueue_scripts( $screen ) {
		...
		if ( in_array( $screen, $tf_options_screens ) || in_array( $post_type, $tf_options_post_type ) ) {
			...
			wp_enqueue_script( 'tf-admin', TF_ASSETS_ADMIN_URL . 'js/tourfic-admin-scripts'. $this->js_min .'.js', array( 'jquery', 'wp-data', 'wp-editor', 'wp-edit-post', 'wp-hooks' ), TF_VERSION, true );
		...
		wp_localize_script( 'tf-admin', 'tf_options', array(
			...
			'tour_traveler_category' => $enable_traveler_category == '1' && ! empty( $tour_traveler_category ) ? $tour_traveler_category : [],
		) );
	}
