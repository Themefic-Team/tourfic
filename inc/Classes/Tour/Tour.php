...
?>
		...
		<?php } elseif (!empty($design) && 4 == $design) { ?>
			<form class="tf-archive-search-box-wrapper tf_tour-shortcode-design-4 <?php echo esc_attr($classes); ?> tf-shortcode-design-4" id="tf_tour_aval_check" method="get" autocomplete="off" action="<?php echo esc_url(Helper::tf_booking_search_action()); ?>">
												...
												<label for="extra<?php echo esc_attr($extrakey); ?>">
													...
													<div class="tf-extra-content">
														<h5><?php echo esc_html($tour_extra['title']); ?> <?php echo $tour_extra_pricetype == "fixed" ? esc_html("(Fixed Price)") : ($tour_extra_pricetype == "person" ? esc_html("(Per Person Price)") : esc_html("(Per unit Price)")); ?>
															<span><?php echo wp_kses_post(wc_price($tour_extra['price'])); ?></span>
														</h5>
														<?php
