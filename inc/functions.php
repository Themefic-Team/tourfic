...
/**
 * Review Admin Notice Feature
 * @author Sydur Rahman
 * @since 2.9.3
 */

// Themefic Plugin Set Admin Notice Status
if(!function_exists('tf_review_activation_status')){

    function tf_review_activation_status(){ 
        $tf_installation_date = get_option('tf_installation_date'); 
        if( !isset($_COOKIE['tf_installation_date']) && empty($tf_installation_date) && $tf_installation_date == 0){
            setcookie('tf_installation_date', 1, time() + (86400 * 7), "/"); 
        }else{
            update_option( 'tf_installation_date', '1' );
        }
    }
    add_action('admin_init', 'tf_review_activation_status');
}

// Themefic Plugin Review Admin Notice
if(!function_exists('tf_review_notice')){
    
     function tf_review_notice(){ 
        ...
        ?>
            <div class="notice notice-info themefic_review_notice"> 
                ...
                <?php echo sprintf( 
                        __( ' <p>Hey %1$s 👋, You have been using <b>%2$s</b> for quite a while. If you feel %2$s is helping your business to grow in any way, would you please help %2$s to grow by simply leaving a 5* review on the WordPress Forum?', 'tourfic' ),
                        $current_user->display_name,
                        'Tourfic'
                    ); ?> 
                
                <ul>
                    <li><a target="_blank" href="<?php echo esc_url('https://wordpress.org/support/plugin/tourfic/reviews/#new-post') ?>" class=""><span class="dashicons dashicons-external"></span><?php _e(' Ok, you deserve it!', 'tourfic' ) ?></a></li>
                    <li><a href="#" class="already_done" data-status="already"><span class="dashicons dashicons-smiley"></span> <?php _e('I already did', 'tourfic') ?></a></li>
                    <li><a href="#" class="later" data-status="later"><span class="dashicons dashicons-calendar-alt"></span> <?php _e('Maybe Later', 'tourfic') ?></a></li>
                    <li><a target="_blank"  href="<?php echo esc_url('https://themefic.com/docs/tourfic/') ?>" class=""><span class="dashicons dashicons-sos"></span> <?php _e('I need help', 'tourfic') ?></a></li>
                    <li><a href="#" class="never" data-status="never"><span class="dashicons dashicons-dismiss"></span><?php _e('Never show again', 'tourfic') ?> </a></li> 
                </ul>
            ...
            <script>
                jQuery(document).ready(function($) {
                    $(document).on('click', '.already_done, .later, .never', function( event ) {
                        ...
                        data = {
                            action : 'tf_review_notice_callback',
                            status : status,
                        };
     ...
     $tf_review_notice_status = get_option('tf_review_notice_status'); 
     $tf_installation_date = get_option('tf_installation_date'); 
     if(isset($tf_review_notice_status) && $tf_review_notice_status <= 0 && $tf_installation_date == 1 && !isset($_COOKIE['tf_review_notice_status']) && !isset($_COOKIE['tf_installation_date'])){ 
        add_action( 'admin_notices', 'tf_review_notice' );  
     }
...
if(!function_exists('tf_review_notice_callback')){

    function tf_review_notice_callback(){
        $status = $_POST['status'];
        if( $status == 'already'){ 
            update_option( 'tf_review_notice_status', '1' );
        }else if($status == 'never'){ 
            update_option( 'tf_review_notice_status', '2' );
        }else if($status == 'later'){
            $cookie_name = "tf_review_notice_status";
            $cookie_value = "1";
            setcookie($cookie_name, $cookie_value, time() + (86400 * 7), "/"); 
            update_option( 'tf_review_notice_status', '0' ); 
        }  
        wp_die();
    }
    add_action( 'wp_ajax_tf_review_notice_callback', 'tf_review_notice_callback' );
