...
function tf_vendor_bulk_filter_callback(){
	...
	if(sanitize_text_field( $_POST['balkaction'] )=="approved"){
		foreach($_POST['vendorlist'] as $single){
			...
			/** Vendor Tours */
			$tours = get_posts( [ 'post_type' => 'tf_tours', 'numberposts' => - 1, 'author' =>  $single['value'], 'post_status' => 'pending' ] );
			if(!empty($tours)){
				foreach ( $tours as $tour ) {
			...
			/** Vendor Hotels */
			$hotels = get_posts( [ 'post_type' => 'tf_hotel', 'numberposts' => - 1, 'author' =>  $single['value'], 'post_status' => 'pending' ] );
			if(!empty($hotels)){
				foreach ( $hotels as $hotel ) {
	...
	if(sanitize_text_field( $_POST['balkaction'] )=="pending"){
		foreach($_POST['vendorlist'] as $single){
			update_user_meta($single['value'], "tf_vendor_approval", "disabled");
...
/**
 * Vendor Admin Ajax registration function
 *
 */
add_action( 'wp_ajax_tf_vendor_registration', 'tf_vendor_registration_callback' );
add_action( 'wp_ajax_nopriv_tf_vendor_registration', 'tf_vendor_registration_callback' );
function tf_vendor_registration_callback() {
	...
	if ( ! isset( $_POST['tf_reg_nonce'] ) || ! wp_verify_nonce( $_POST['tf_reg_nonce'], 'check_reg_nonce' ) ) {
		return;
	}
	...
	// User error check
	if ( empty( $username ) ) {
		$reg_errors->add( 'empty_user', __( 'Please Enter a Username.', 'tourfic' ) );
	} else {
		if ( username_exists( $username ) ) {
			$reg_errors->add( 'exist_user', __( 'Username already exists, please try another.', 'tourfic' ) );
		}
		if ( ! validate_username( $username ) ) {
			$reg_errors->add( 'invalid_user', __( 'Please Enter a valid Username.', 'tourfic' ) );
		}
	}
	// Email error check
	if ( empty( $useremail ) ) {
		$reg_errors->add( 'empty_email', __( 'Please Enter an Email.', 'tourfic' ) );
	} else {
		if ( ! is_email( $useremail ) ) {
			$reg_errors->add( 'invalid_email', __( 'Please Enter a valid Email.', 'tourfic' ) );
		}
		if ( email_exists( $useremail ) ) {
			$reg_errors->add( 'exist_email', __( 'This email address is already in use.', 'tourfic' ) );
		}
	}

	// Password error check
	if ( empty( $password ) ) {
		$reg_errors->add( 'empty_password', __( 'Please Enter Password.', 'tourfic' ) );
	} else {
		if ( 0 === preg_match( "/.{6,}/", $password ) ) {
			$reg_errors->add( 'invalid_password', __( 'Password must be at least six characters.', 'tourfic' ) );
		}
	}
	// User role error check
	if ( empty( $user_role ) ) {
		$reg_errors->add( 'empty_role', __( 'Register type must be selected.', 'tourfic' ) );
	}
	...
	if ( is_wp_error( $reg_errors ) ) {
		foreach ( $reg_errors->get_error_messages() as $error ) {
			echo '<div class="tf_errors">' . $error . '</div>';
		}
	}
	...
	if ( 1 > count( $reg_errors->get_error_messages() ) ) {
		...
		$user_email = sanitize_email( $useremail );
		$user_first_name = sanitize_text_field( $first_name );
		$user_last_name = sanitize_text_field( $last_name );
		...
		$userdata = array(
			...
			'user_pass'  => $password,
			'role'       => !empty($user_role) ? $user_role : 'tf_vendor',
			'first_name' => !empty($user_first_name) ? $user_first_name : '',
			'last_name' => !empty($user_last_name) ? $user_last_name : '',
		);

		$user_id = wp_insert_user( $userdata );
		if(!empty($phone)){
			update_user_meta($user_id, 'tf_user_phone', $phone);
		}
		if(!empty($vendor_status) && $vendor_status==1){
			update_user_meta($user_id, "tf_vendor_approval", "enabled");
		}else{
			update_user_meta($user_id, "tf_vendor_approval", "disabled");
		}
		// If successfull
...
/**
 * Vendor Admin Ajax update function
 *
 */
add_action( 'wp_ajax_tf_vendor_update', 'tf_vendor_update_callback' );
add_action( 'wp_ajax_nopriv_tf_vendor_update', 'tf_vendor_update_callback' );
function tf_vendor_update_callback() {
	...
	if ( 1 > count( $reg_errors->get_error_messages() ) ) {
		...
		if(!empty($vendor_status) && $vendor_status==1){
			...
			/** Vendor Tours */
			$tours = get_posts( [ 'post_type' => 'tf_tours', 'numberposts' => - 1, 'author' =>  $user_id ] );
			if(!empty($tours)){
				foreach ( $tours as $tour ) {
					$vendor_tours_pending = array(
						'ID'            => $tour->ID,
			...
			/** Vendor Hotels */
			$hotels = get_posts( [ 'post_type' => 'tf_hotel', 'numberposts' => - 1, 'author' =>  $user_id ] );
			if(!empty($hotels)){
				foreach ( $hotels as $hotel ) {
					$vendor_hotels_pending = array(
						'ID'            => $hotel->ID,
		...
		// If successfull
		if ( ! is_wp_error( $user_id ) ) {
			echo sprintf( '<div class="tf_success">%s</div>', __( 'Successfully Updated.', 'tourfic' ) );
		}
