 ...
 */
function tf_save_rating( int $comment_id ) {
...
add_action( 'comment_post', 'tf_save_rating', 10, 1 );
/**
 * 3. Making the rating required (optional)
 *
 */
...
function tf_comment_callback( $comment, $args, $depth ) {
    ...
    <div class="tf-author-infobox">
        <div class="comment-meta commentmetadata">
            ...
            <p><?php echo $tf_overall_rate ?>/5</p>
        </div>
    </div>
 ...
 * @param null|int $post_id current post type
 *
 * @return string
 */
function tf_average_ratings( array $ratings = [], int $post_id = null ): string {
	if ( ! $ratings ) {
		return 'N/A';
	}
...
function tf_calculate_user_ratings( $comment, array &$overall_rating ): void {
	...
	if ( $tf_comment_meta ) {
		...
		foreach ( $tf_comment_meta as $key => $value ) {
			$overall_ratin1[ $key ][] = $value ? $value : "0";
		}
		foreach ( $overall_ratin1 as $key => $ratings ) {
 ...
 * @param array $data
 */
function tf_update_overall_rating_of_post( int $comment_ID, array $data ) {
	$comments       = get_comments( [ 'post_id' => $data['comment_post_ID'], 'status' => 'approve' ] );
	$overall_rating = [];
	foreach ( $comments as $comment ) {
		tf_calculate_user_ratings( $comment, $overall_rating );
	}
	update_post_meta( $data['comment_post_ID'], 'tf_overall_review', $overall_rating );
}

add_action( 'edit_comment', 'tf_update_overall_rating_of_post', 10, 2 );
