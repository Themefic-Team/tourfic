...
?>
    ...
    <div class="price-per-night"><?php esc_html_e('Price per night', TFD); ?></div>
    ...
    <div id="tf-ask-question" style="display: none;">
        ...
        <div class="tf-aq-outer">
            ...
            <div class="tf-aq-inner">
                <h4><?php esc_html_e('Submit your question', TFD); ?></h4>
                <form id="ask-question" action="" method="post">
                    <div class="tf-aq-field">
                        <input type="text" name="your-name" placeholder="<?php esc_attr_e('Your Name', TFD); ?>" required />
                    </div>
                    <div class="tf-aq-field">
                        <input type="email" name="your-email" placeholder="<?php esc_attr_e('Your email', TFD); ?>" required />
                    </div>
                    <div class="tf-aq-field">
                        <textarea placeholder="<?php esc_attr_e('Your Question', TFD); ?>" name="your-question" required></textarea>
                    </div>
                    <div class="tf-aq-field">
                        <button type="reset" class="screen-reader-text"><?php esc_html_e('Reset', TFD); ?></button>
                        <button type="submit" form="ask-question" class="button tf_button"><?php esc_html_e('Submit', TFD); ?></button>
    ...
    <?php
        ...
        function tourfic_ask_question_ajax()
        {
            ...
            if (!check_ajax_referer('ask_question_nonce')) {
                ...
                $response['msg'] = __('Security error! Reload the page and try again.', TFD);
            ...
            $subject     = sprintf(esc_html__('Someone asked question on: %s', TFD), $post_title);
            $message     = "{$question}";
            ...
            if (wp_mail($send_email_to, $subject, $message, $headers, $attachments)) {
                ...
                $response['msg'] = __('Your question has been sent!', TFD);
            } else {
                $response['status'] = 'error';
                $response['msg'] = __('Message sent failed!', TFD);
            }
