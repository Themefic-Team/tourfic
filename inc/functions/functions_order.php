...
if ( ! function_exists( 'tf_apartment_booking_page_callback' ) ) {
	function tf_apartment_booking_page_callback() {
		...
		// Query
		if ( $current_user_role == 'administrator' ) {
			$query_orders = wc_get_orders( array( 'numberposts' => $limit, 'paginate' => true, '_order_type' => 'apartment', 'paged' => $pagenum ) );
		}
		if ( $current_user_role == 'tf_vendor' ) {
			$query_orders = wc_get_orders( array( 'numberposts' => $limit, 'paginate' => true, '_order_type' => 'apartment', '_post_author' => $current_user_id, 'paged' => $pagenum ) );
		}
		$offset = ( $pagenum - 1 ) * $limit;
        ...
        <div class="wrap" style="margin-right: 20px;">
			...
			<?php do_action( 'tf_before_apartment_booking_details' ); ?>
            <hr class="wp-header-end">
            <form id="posts-filter">
                <div class="tablenav top">
                ...
                <table class="wp-list-table widefat fixed striped table-view-list pages tf-order-data-table">
                    ...
                    <tr>
                        ...
                        <th class="manage-column sortable" style="width: 27%;">
                            <a href="#"><span><?php _e( 'Customer Details', 'tourfic' ); ?></span><span class="sorting-indicator"></span></a>
                        </th>
                        <th class="manage-column sortable" style="width: 27%;">
                            <a href="#"><span><?php _e( 'Order Details', 'tourfic' ); ?></span><span class="sorting-indicator"></span></a>
						...
						<?php if ( $current_user_role == 'administrator' ) { ?>
                            <th class="manage-column sortable" style="width: 4%;"></th>
						<?php } ?>
                    </tr>
                    </thead>
					...
					foreach ( $orders as $key => $order ) {
						if ( function_exists( 'is_tf_pro' ) && is_tf_pro() ) {
							tf_apartment_order_single_row( $order );
						} else {
							if ( $key == 14 ) {
								tf_apartment_order_single_row( $order );
								echo '<tr class="pro-row" style="text-align: center; background-color: #ededf8"><td colspan="9"><a href="https://tourfic.com/" target="_blank"><h3 class="tf-admin-btn tf-btn-secondary" style="color:#fff;margin: 15px 0;">' . __( 'Upgrade to Pro Version to see more', 'tourfic' ) . '</h3></a></td></tr>';
...
if ( ! function_exists( 'tf_apartment_order_single_row' ) ) {
	function tf_apartment_order_single_row( $order ) {
        ...
        <tr id="" class="iedit author-self level-0 type-page status-publish hentry entry">
            ...
            <td>
				...
				foreach ( $order->get_items() as $item_key => $item_values ) {
					...
					$check_in_out = ! empty( wc_get_order_item_meta( $item_key, 'check_in_out_date', true ) ) ? wc_get_order_item_meta( $item_key, 'check_in_out_date', true ) : '';
					$adult        = ! empty( wc_get_order_item_meta( $item_key, 'adults', true ) ) ? wc_get_order_item_meta( $item_key, 'adults', true ) : '';
					$children     = ! empty( wc_get_order_item_meta( $item_key, 'children', true ) ) ? wc_get_order_item_meta( $item_key, 'children', true ) : '';
					$infant       = ! empty( wc_get_order_item_meta( $item_key, 'infant', true ) ) ? wc_get_order_item_meta( $item_key, 'infant', true ) : null;
					echo '<b>' . __( "Apartment Name", "tourfic" ) . ': </b><a href="' . esc_url( get_permalink( $post_id ) ) . '" target="_blank">' . esc_html( get_the_title( $post_id ) ) . '</a><br>';
					...
					if ( $check_in_out ) {
						echo '<b>' . __( "Check-in-out", "tourfic" ) . ': </b>' . $check_in_out . '<br>';
					}
