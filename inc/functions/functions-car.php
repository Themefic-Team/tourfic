...
function tf_add_account_refund_order_action( $actions, $order ) {
	...
	foreach ( $order->get_items() as $item_id => $item ) {
		...
		if('car'==$order_type){
			$order_pickup_date = $item->get_meta( 'Pick Up Date', true );
			$order_pickup_time = $item->get_meta( 'Pick Up Time', true );
			...
			if(!empty($bestRefundPolicy)){
				$today = new DateTime(); // Current date and time

				// Combine pickup date and time into a single string and convert it to a DateTime object
				$pickupDateTime = DateTime::createFromFormat('Y/m/d H:i', $order_pickup_date . ' ' . $order_pickup_time);
    ...
    if ( $order->is_paid() && $less_current_day) {
        $actions['tf_refund_request'] = array(
            'url'  => tf_get_order_refund_request_url( $order ),
            'name' => __( 'Request Refund', 'tourfic' ),
        );
    }

    return $actions;
}
add_filter( 'woocommerce_my_account_my_orders_actions', 'tf_add_account_refund_order_action', 10, 2 );

function tf_get_order_refund_request_url( $order ) {

    // enter the ID for your page that contains the refund request form
    $page = get_page_by_path( 'tf-refund-page' );
    $page_id = $page->ID;
    ...
    if ( ! empty( $products ) ) {
        $refund_url .= '&order-type=' . urlencode( implode( ', ', $products ) );
    }

    return $refund_url;
}
function tf_add_refund_request_email_link( $order ) {
    if ( $order->is_paid() ) {
        echo '<br /><a class="link" href="' . esc_url( tf_get_order_refund_request_url( $order ) ) . '">Request Refund</a>';
    }
}
add_action( 'woocommerce_email_after_order_table', 'tf_add_refund_request_email_link', 15, 1 );


function tf_register_request_refund_order_status() {
...
add_action( 'init', 'tf_register_request_refund_order_status' );

function tf_add_awaiting_shipment_to_order_statuses( $order_statuses ) {
    ...
    foreach ( $order_statuses as $key => $status ) {
        ...
        if ( 'wc-processing' === $key ) {
            $new_order_statuses['wc-refund-request'] = 'Refund Requested';
        }
    }

    return $new_order_statuses;
}
add_filter( 'wc_order_statuses', 'tf_add_awaiting_shipment_to_order_statuses' );




add_action( 'wp_ajax_tf_customer_refund_request', 'tf_customer_refund_request_callback' );

function tf_customer_refund_request_callback(){
    ...
    if (!empty($order)) {
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			if('car'==$order_type){
				...
				$order->update_meta_data( '_tf_refundable_amount', wc_price($price) );
			}
			break;
		}

		$order->update_status( 'wc-refund-request' );
		$order->save();
		...
		$mailer['WC_Email_Customer_Completed_Order']->trigger( $_POST['order'] );
		
		

		//redirect customer profile order page
		$tf_sucessfull_page = home_url().'/my-account/orders';
		$response['redirect_to'] = $tf_sucessfull_page;
		echo wp_json_encode( $response );
...
// Hook to display Refundable Amount in the WooCommerce admin order details page
add_action( 'woocommerce_admin_order_totals_after_total', 'tf_display_refundable_amount_admin_order', 10, 1 );
function tf_display_refundable_amount_admin_order( $order_id ) {

    // Get the order object
    $order = wc_get_order( $order_id );

    // Retrieve the Refundable Amount from order meta
    $refundable_amount = $order->get_meta( '_tf_refundable_amount' );

    // If there is a refundable amount, display it
    if ( ! empty( $refundable_amount ) ) {
        ?>
        <tr>
            <td class="label" style="color: red"><?php _e( 'Refundable Amount', 'tourfic' ); ?>:</td>
            <td width="1%"></td>
            <td class="total" style="color: red"><?php echo $refundable_amount; ?></td>
        </tr>
        <?php
    }
}
