...
if (!is_plugin_active('tourfic-pro/tourfic-pro.php')) {
    function tf_add_pro_link_menu()
    {
        ...
        add_submenu_page(TFD, __('Upgrade to Pro', TFD), $menuname, 'manage_options', $prolink);
    }
...
function tf_plugin_action_links($links)
{
    ...
    $settings_link = array(
        '<a href="admin.php?page=tourfic">' . esc_html__('Settings', TFD) . '</a>',
    );

    if (!is_plugin_active('tourfic-pro/tourfic-pro.php')) {
        $gopro_link = array(
            '<a href="https://tourfic.com/go/upgrade" target="_blank" style="color:#cc0000;font-weight: bold;text-shadow: 0px 1px 1px hsl(0deg 0% 0% / 28%);">' . esc_html__('GO PRO', TFD) . '</a>',
        );
