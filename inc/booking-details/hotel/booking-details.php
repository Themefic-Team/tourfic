        ...
        <div class="tf-filter-options">
            <div class="tf-order-status-filter">
                <select class="tf-tour-filter-options tf-hotel-id-filter-options">
                    <option value=""><?php esc_html_e("Hotel name", "tourfic"); ?></option>
                    <?php 
                    $tf_hotel_list = array(
                        'posts_per_page' => - 1,
                        'post_type'      => 'tf_hotel',
    ...
    <table class="wp-list-table table" cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                ...
                <th id="odetails">
                    <?php esc_html_e("Hotel name", "tourfic"); ?>
                </th>
                <th id="cdetails">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                <a href="<?php echo esc_url( admin_url( 'edit.php?post_type=tf_hotel&page=tf_hotel_booking&order_id=' . esc_attr( $hotel['order_id'] ) . '&book_id=' . esc_attr( $hotel['id'] ) . '&action=preview' ) ); ?>">
                    <?php echo esc_html( $hotel['order_id'] ); ?>
                </a>
                </td>
                <td>
                    <?php echo esc_html(get_the_title($hotel['post_id'])); ?>
                </td>
                <td>
                    <?php 
                ...
                <td>
                    <?php echo esc_html(gmdate('F d, Y',strtotime($hotel['order_date']))); ?>
                </td>
                <td>
                    <?php 
                ...
                <td>
                    <?php 
                    $actions_details = '<a href="'.admin_url().'edit.php?post_type=tf_hotel&amp;page=tf_hotel_booking&amp;order_id='.$hotel['order_id'].'&amp;book_id='.$hotel['id'].'&amp;action=preview&nonce='.esc_attr( wp_create_nonce( 'tf_booking_details' ) ).'" class="tf_booking_details_view"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
