...
class Rooms {
	...
	public static function render( $settings = [], $builder = '' ) {
		...
		if ( 'tf_hotel' === $post_type ) {
            self::render_hotel_rooms( $settings, $builder );
		} elseif ( 'tf_apartment' === $post_type ) {
			self::render_apartment_rooms( $settings, $builder );
		}

		echo ! empty( $wrapper_close ) ? wp_kses_post( $wrapper_close ) : '';
	}

	static private function render_hotel_rooms($settings, $builder) {
        $post_id       = get_the_ID();
		...
		$room_display = apply_filters(
			'tourfic_hotel_room_display',
			array(
				'show_table_price' => true,
				'show_card_price'  => true,
				'actions'          => array(),
			),
			array(
				'post_id' => $post_id,
		...
		$show_table_price = ! isset( $room_display['show_table_price'] ) || (bool) $room_display['show_table_price'];
		$show_card_price  = ! isset( $room_display['show_card_price'] ) || (bool) $room_display['show_card_price'];
		$room_actions     = ! empty( $room_display['actions'] ) && is_array( $room_display['actions'] ) ? $room_display['actions'] : array();
		...
		if ($style == 'style1' && $rooms) {
                                                ...
                                                <div class="tf-room-preview-img">
                                                    ...
                                                    if ( $tour_room_details_gall ) {
                                                        ...
                                                        data-hotel="<?php echo esc_attr( $post_id ); ?>">
                                                            <img src="<?php echo esc_url( $room_preview_img ); ?>" alt="<?php esc_attr_e( "Room Image", "tourfic" ); ?>">
                                                ...
                                                <ul>
                                                    ...
                                                    if ( ! empty( $room['features'] ) ) {
                                                        if ( count( $room['features'] ) > 3 ) {
                                                            echo '<span>' . esc_html__( 'More....', 'tourfic' ) . '</span>';
                                                        }
                                                    }
                                                    ?>
                                                </ul>
                                    ...
                                    if ( $pricing_by == '3' && ! empty( $room_options ) ):
                                        foreach ( $room_options as $room_option_key => $room_option ):
                                            ...
                                            <td class="reserve tf-t-c">
                                                <?php if ( $show_table_price ) {
                                                    Pricing::instance( get_the_ID(), $_room->ID )->get_per_price_html( $room_option_key );
                                                } ?>
                                                <?php
                                                self::render_hotel_room_action(
                                                    $room_actions,
                                                    'style1-option',
                                                    '<button class="hotel-room-availability tf_btn tf_btn_gray" type="submit" style="margin: 0 auto;">' . esc_html__( 'Check Availability', 'tourfic' ) . '</button>'
                                                );
                                                ?>
                                            </td>
                                            </tr>
                                    ...
                                    <?php endif; ?>
                                        ...
                                        <td class="reserve tf-t-c">
                                            ...
                                            self::render_hotel_room_action(
                                                $room_actions,
                                                'style1',
                                                '<button class="hotel-room-availability tf_btn tf_btn_gray" type="submit">' . esc_html__( 'Check Availability', 'tourfic' ) . '</button>'
                                            );
                                            ?>
                                        </td>
                                        </tr>
            ...
            ?>
			<?php echo 'yes' === $wrapper ? '<div class="tf-single-template__two tf-single-hotel-room__style-2">' : ''; ?>
                                                    ...
                                                    <div class="tf-room-options-content-inner">
                                                        <div class="tf-available-room-content-left">
                                                            ...
                                                            <ul class="tf-room-info-list">
                                                                ...
                                                                <li><a href="#" class="tf-room-detail-popup" data-uniqid="<?php echo ! empty( $room['unique_id'] ) ? esc_attr( $room['unique_id'] . $room_id ) : '' ?>"
                                                                    data-design="design-2" data-hotel="<?php echo esc_attr( $post_id ); ?>"><?php esc_html_e( "View room details", "tourfic" ); ?></a></li>
                                                            ...
                                                            <ul>
                                                                ...
                                                                if ( ! empty( $room['features'] ) ) {
                                                                    if ( count( $room['features'] ) >= 6 ) {
                                                                        ...
                                                                        <li><a href="#" class="tf-room-detail-popup" data-uniqid="<?php echo ! empty( $room['unique_id'] ) ? esc_attr( $room['unique_id'] . $room_id ) : '' ?>"
                                                                            data-design="design-2" data-hotel="<?php echo esc_attr( $post_id ); ?>"><?php esc_html_e( "See all benefits", "tourfic" ); ?></a></li>
                                                                        <?php
                                                                    }
                                                                }
                                                                ?>
                                                        ...
                                                        <div class="tf-available-room-content-right">
                                                            ...
                                                            self::render_hotel_room_action(
                                                                $room_actions,
                                                                'style2-option',
                                                                '<a href="#room-availability" class="tf_btn tf_btn_large tf_btn_sharp">' . esc_html__( 'Check Availability', 'tourfic' ) . '</a>'
                                                            );
                                                            ?>
                                                        </div>
                                                    </div>
            ...
            ?>
            <?php echo 'yes' === $wrapper ? '<div class="tf-single-template__legacy tf-single-hotel-room__style-legacy sp-0">' : ''; ?>
                                                ...
                                                <td class="reserve tf-t-c">
                                                    <div class="tf-btn-wrap">
                                                        ...
                                                        self::render_hotel_room_action(
                                                            $room_actions,
                                                            'style3-option',
                                                            '<button class="tf_btn tf_btn_full hotel-room-availability" type="submit">' . esc_html__( 'Check Availability', 'tourfic' ) . '</button>'
                                                        );
                                                        ?>
                                                    </div>
                                                </td>
	...
	/**
	 * Render a room action supplied by an extension or use the Free default.
	 *
	 * @param array  $actions      Registered room actions.
	 * @param string $surface      Render surface identifier.
	 * @param string $default_html Default Free action markup.
	 */
	private static function render_hotel_room_action( $actions, $surface, $default_html ) {
		$action = ! empty( $actions[ $surface ] ) && is_array( $actions[ $surface ] ) ? $actions[ $surface ] : array();
		if ( empty( $action['url'] ) ) {
			echo wp_kses_post( $default_html );
			return;
		}

		$label  = ! empty( $action['label'] ) ? $action['label'] : esc_html__( 'Book Now', 'tourfic' );
		$class  = ! empty( $action['class'] ) ? $action['class'] : 'tf_btn';
		$target = ! empty( $action['target'] ) ? $action['target'] : '';
		?>
		<a href="<?php echo esc_url( $action['url'] ); ?>" class="<?php echo esc_attr( $class ); ?>"<?php echo $target ? ' target="' . esc_attr( $target ) . '"' : ''; ?>><?php echo esc_html( $label ); ?></a>
		<?php
	}

	static private function render_apartment_rooms($settings, $builder) {
        $style = !empty($settings['room_style']) ? $settings['room_style'] : 'style1';
        $container = ! empty( $settings['container'] ) ? $settings['container'] : 'no';
        $wrapper = ! empty( $settings['wrapper'] ) ? $settings['wrapper'] : 'yes';
		$post_id = get_the_ID();
		...
		if ($style == 'style1' && ! empty( Helper::tf_data_types( $meta['rooms'] ) )) {
            ?>
			<?php echo 'yes' === $wrapper ? '<div class="tf-single-template__two tf-single-apartment-room__style-1">' : ''; ?>
            ...
            ?>
            <?php echo 'yes' === $wrapper ? '<div class="tf-single-template__legacy tf-single-apartment-room__style-legacy sp-0">' : ''; ?>
