...
/**
 * Single Car Info Component
 * Shared markup for Elementor and Bricks Car Info widgets
 */
class Car_Info {

	/**
	 * Static render method for Car Info component
	 *
	...
	public static function render( $settings = [], $builder = '' ) {
		...
		<div class="tf-car-info" id="tf-car-info">
			<?php if ( ! empty( $car_info_title ) ) { ?>
				<h3 class="tf-section-title"><?php echo esc_html( $car_info_title ); ?></h3>
			<?php } ?>
						...
						if ( ! empty( $info['title'] ) ) { ?>
							<li class="tf-flex tf-flex-align-center tf-flex-gap-6">
								...
								<?php if ( ! empty( $info['content'] ) ) { ?>
									<div class="tf-car-info-tooltip">
										<span><?php echo esc_html( $info['content'] ); ?></span>
									</div>
								<?php } ?>
							</li>
						<?php }
