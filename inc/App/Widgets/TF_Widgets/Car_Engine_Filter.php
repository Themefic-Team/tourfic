 ...
 * Works only for Tour
 */
class Car_Engine_Filter extends \WP_Widget {
    ...
    public function __construct() {
        ...
        parent::__construct(
            'tf_car_engine_filter', // Base ID
            esc_html__( 'Tourfic - Car Filters by Engine Year', 'tourfic' ),
            array( 'description' => esc_html__( 'Filter search result by car Engine', 'tourfic' ) ) // Args
        );
    }
    ...
    public function widget( $args, $instance ) {
        ...
        if ( $posttype == 'tf_carrental' ) {
            ...
            foreach ( $get_terms as $key => $term ) {
                ...
                echo wp_kses("<li class='filter-item'><label><input type='checkbox' name='car_engine_year[]' value='{$id}'/><span class='checkmark'></span> {$name}</label> {$count}</li>", Helper::tf_custom_wp_kses_allow_tags());
            }
            echo "</ul></div>";
    ...
    public function form( $instance ) {
        ...
        $title = isset( $instance['title'] ) ? $instance['title'] : esc_html__( 'Make Year', 'tourfic' );
