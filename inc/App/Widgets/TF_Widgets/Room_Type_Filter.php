 ...
 * @author Foysal
 */
class Room_Type_Filter extends \WP_Widget {
    ...
    public function __construct() {
        ...
        parent::__construct(
            'tf_room_type_filter', // Base ID
            esc_html__( 'Tourfic - Rooms Filters by Type', 'tourfic' ),
            array( 'description' => esc_html__( 'Filter search result by room type', 'tourfic' ) ) // Args
        );
    }
    ...
    public function widget( $args, $instance ) {
        ...
        if ( is_admin() || $posttype == 'tf_room' ) {
            ...
            if ( ! empty( $terms ) && is_array( $terms ) ) {
                $taxonomy['include'] = $terms;
            }

            $get_terms = get_terms( $taxonomy );

            $destination_name  = isset( $tourfic_search_request['destination'] ) ? $tourfic_search_request['destination'] : '';
            $search_types_query = isset( $tourfic_search_request['room_types'] ) ? $tourfic_search_request['room_types'] : array();
            ...
            foreach ( $get_terms as $key => $term ) {
                ...
                $count = $show_count ? '<span>' . Room::tf_term_count( $term->slug, $destination_name, $default_count ) . '</span>' : '';
                $defult_select =  in_array($fslug, $search_types_query) ? 'checked' : '';
                echo wp_kses("<li class='tf-filter-item'><label><input type='checkbox' name='tf_room_types[]' value='{$id}' {$defult_select} /><span class='tf-checkmark'></span> {$name}</label> {$count}</li>", Helper::tf_custom_wp_kses_allow_tags());
            }
    ...
    public function form( $instance ) {
        ...
        $title = isset( $instance['title'] ) ? $instance['title'] : esc_html__( 'Room Types', 'tourfic' );
