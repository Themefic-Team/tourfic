    ...
    public function widget( $args, $instance ) {
        ...
        if ( is_admin() || $posttype == 'tf_hotel' ) {
            ...
            foreach ( $get_terms as $key => $term ) {
                ...
                echo wp_kses("<li class='tf-filter-item'><label><input type='checkbox' name='tf_hotel_types[]' value='{$id}' {$defult_select} /><span class='tf-checkmark'></span> {$name}</label> {$count}</li>", Helper::tf_custom_wp_kses_allow_tags());
            }
