    ...
    public function widget( $args, $instance ) {
        ...
        if ( is_admin() || $posttype == 'tf_tours' ) {
            ...
            foreach ( $get_terms as $key => $term ) {
                ...
                echo wp_kses("<li class='tf-filter-item'><label><input type='checkbox' name='tour_features[]' value='{$id}' /><span class='tf-checkmark'></span> {$name}</label> {$count}</li>", Helper::tf_custom_wp_kses_allow_tags());
            }
