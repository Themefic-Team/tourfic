    ...
    public function widget( $args, $instance ) {
        ...
        if( is_post_type_archive('tf_tours') || is_post_type_archive('tf_hotel') || is_post_type_archive('tf_apartment') || is_post_type_archive('tf_carrental') || is_post_type_archive('tf_room') || ( !empty( $tf_query_taxonomy ) ) ){
            ...
            ?>
                ...
                <div class="tf-room-result-price-range"></div>
            <?php
            } 
            ...
            if( !is_post_type_archive('tf_hotel') && !is_post_type_archive('tf_tours') && !is_post_type_archive('tf_apartment') && !is_post_type_archive('tf_carrental') && !is_post_type_archive('tf_room') && ( !empty(get_taxonomy(get_queried_object()->taxonomy)->object_type) && get_taxonomy(get_queried_object()->taxonomy)->object_type[0]=="tf_room" ) ){
                ?>
                    <div class="tf-widget-title">
                        <span><?php esc_html_e("Price Range","tourfic"); ?> (<?php echo wp_kses_post(get_woocommerce_currency_symbol()); ?>)</span>
                    </div>
            ...
            ?>
                ...
                <div class="tf-hotel-result-price-range"></div>
            <?php }
            if( "tf_room" === $tourfic_search_type && ! empty( $tourfic_search_request['from'] ) && ! empty( $tourfic_search_request['to'] ) ){
            ?>
                <div class="tf-widget-title">
                    <span><?php esc_html_e("Price Range","tourfic"); ?> (<?php echo wp_kses_post(get_woocommerce_currency_symbol()); ?>)</span>
                </div>
            ...
            ?>
                ...
                <div class="tf-apartment-result-price-range"></div>
            <?php }
            if( "tf_carrental" === $tourfic_search_type && ! empty( $tourfic_search_request['from'] ) && ! empty( $tourfic_search_request['to'] ) ){
            ?>
                <div class="tf-widget-title">
                    <span><?php esc_html_e("Price Range","tourfic"); ?> (<?php echo wp_kses_post(get_woocommerce_currency_symbol()); ?>)</span>
                </div>
