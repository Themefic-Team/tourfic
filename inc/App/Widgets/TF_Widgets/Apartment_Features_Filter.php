...
class Apartment_Features_Filter extends \WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( is_admin() || 'tf_apartment' === $posttype ) {
			...
			Apartment::tf_sync_legacy_apartment_feature_terms();
			...
			if ( isset( $_GET['features'] ) ) {
				if ( is_array( $_GET['features'] ) ) {
					...
					$search_types_query[] = sanitize_text_field( wp_unslash( $_GET['features'] ) );
				}
			}
			...
			foreach ( $get_terms as $key => $term ) {
				...
				echo wp_kses("<li class='tf-filter-item'><label><input type='checkbox' name='tf_apartment_features[]' value='{$id}' {$defult_select} /><span class='tf-checkmark'></span> {$name}</label> {$count}</li>", Helper::tf_custom_wp_kses_allow_tags() );
			}
