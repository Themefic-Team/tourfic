 ...
 * @author Abu Hena
 */
class Tour_Activities_Filter extends \WP_Widget {
    ...
    public function widget( $args, $instance ) {
        ...
        if ( is_admin() || $posttype == 'tf_tours' ) {
            ...
            foreach ( $get_terms as $key => $term ) {
                ...
                echo wp_kses("<li class='tf-filter-item'><label><input type='checkbox' name='tf_activities[]' value='{$id}' /><span class='tf-checkmark'></span> {$name}</label> {$count}</li>", Helper::tf_custom_wp_kses_allow_tags());
            }
