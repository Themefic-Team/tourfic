...
class Hotel_Booking_Details extends \Tourfic\Admin\Booking_Details\Booking_Details {
	...
	public function __construct() {
		...
		add_submenu_page( 'edit.php?post_type=tf_hotel',
			__( 'Hotel Booking Details', 'tourfic' ),
			__( 'Booking Details', 'tourfic' ),
			'edit_tf_hotels',
			'tf_hotel_booking',
			'tf_hotel_booking_page_callback'
		);
	...
	function tf_hotel_booking_page_callback() {
		if ( ! empty( $_GET['order_id'] ) && ! empty( $_GET['action'] ) && ! empty( $_GET['book_id'] ) ) {
            ...
            <div class="wrap tf_booking_details_wrap" style="margin-right: 20px;">
				 ...
				 * @since 2.9.26
				 */
				$this->tf_hotel_booking_details_list();
				?>
            </div>
	...
	function tf_hotel_booking_details_list($type = 'hotel') {
		?>
                ...
                <div class="tf-filter-options">
                    <div class="tf-order-status-filter">
                        <select class="tf-tour-filter-options tf-hotel-id-filter-options">
							...
							$tf_hotel_list_query = new \Tourfic\Admin\Booking_Details\WP_Query( $tf_hotel_list );
                    ...
                    <th colspan="8">
                        <ul class="tf-booking-details-pagination">
							<?php if ( ! empty( $paged ) && $paged >= 2 ) { ?>
                                <li><a href="<?php echo \Tourfic\Admin\Booking_Details\tf_booking_details_pagination( $paged - 1 ); ?>">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
							...
							if ( ! empty( $total_pages ) && $total_pages > 1 ) {
								for ( $i = 1; $i <= $total_pages; $i ++ ) {
									if ( $i == $paged ) {
										?>
                                        <li class="active">
                                            <a href="<?php echo \Tourfic\Admin\Booking_Details\tf_booking_details_pagination( $i ); ?>"><?php echo $i; ?></a>
                                        </li>
									<?php } else { ?>
                                        <li>
                                            <a href="<?php echo \Tourfic\Admin\Booking_Details\tf_booking_details_pagination( $i ); ?>"><?php echo $i; ?></a>
							...
							if ( ! empty( $total_pages ) && ! empty( $paged ) && $paged < $total_pages ) {
								?>
                                <li><a href="<?php echo \Tourfic\Admin\Booking_Details\tf_booking_details_pagination( $paged + 1 ); ?>"><?php _e( "Next", "tourfic" ); ?>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
