...
class Booking_Details {
    ...
    private $booking_args = array();

	public function __construct($booking_args) {
        $this->booking_args = $booking_args;

		add_submenu_page( 'edit.php?post_type='.$this->booking_args['post_type'],
			__( $this->booking_args['menu_title'], 'tourfic' ),
			__( 'Booking Details', 'tourfic' ),
			$this->booking_args['capability'],
			$this->booking_args['menu_slug'],
			'tf_booking_page_callback'
		);

	}

	function tf_booking_page_callback() {
