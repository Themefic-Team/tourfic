...
class Tour_Enquiry extends \Tourfic\Core\Enquiry {
	...
	public function tf_enquiry_page_callback() {
		...
		if ( $enquiry_id || $action ) {
			$this->single_enquiry_details( $this->tf_get_enquiry_for_admin_view( 'tf_tours' ) );
        } else {
			if ( ! $this->tf_current_user_can_manage_enquiry_post_type( 'tf_tours' ) ) {
			...
			if ( $paged > 1 ) {
				check_admin_referer( 'tourfic_filter_enquiries_tf_tours' );
			}
            ...
            ?>
                <div class="wrap tf_booking_details_wrap tf-enquiry-details-wrap" style="margin-right: 20px;">
                    ...
                    <div class="tf_enquiry_details_wrap_header">
						<h1 class="wp-heading-inline"><?php echo  esc_html( apply_filters( 'tourfic_tour_enquiry_page_heading', esc_html__( 'Tour Enquiry Details', 'tourfic' ) ) ); ?></h1>
                        <div class="tf_header_wrap_button">
                            <?php
                                do_action( 'tourfic_before_enquiry_details' ); //old hook
								do_action( 'tourfic_before_tour_enquiry_details' );
								do_action( 'tourfic_after_tour_enquiry_details');
                            ?>
                        </div>
                    </div>
                    <?php 
					...
					$author_id    = $this->tf_current_user_can_manage_all_enquiries( 'tf_tours' ) ? 0 : get_current_user_id();
					$enquiry_data = $this->enquiry_table_data( 'tf_tours', '', '', 0, 0, $author_id );
