...
$files = array(
	...
	'pro_tourfic_js' => dirname( $root ) . '/tourfic-pro/sass/app/js/pro/tourfic-pro.js',
);
...
tf_traveler_compliance_assert(
	false === strpos( $tour_js_source, 'package_start_time' ),
	'Free tour submit must not implement Pro-owned package time handling.'
);
tf_traveler_compliance_assert(
	false !== strpos( $pro_tourfic_js_source, 'const packageTime = selectedPackage' )
		&& false !== strpos( $pro_tourfic_js_source, ".closest('.tf-single-package')" )
		&& false !== strpos( $pro_tourfic_js_source, ".find('select[name=\"package_start_time\"] option:selected')" ),
	'Pro tour submit must read package time from the selected package container.'
);
tf_traveler_compliance_assert(
...
tf_traveler_compliance_assert(
	false === strpos( $tour_js_source . $pro_tourfic_js_source, "\$('#package-' + selectedPackage)" ),
