...
function tf_security_live_route_supports_method( $route, $method ) {
	...
	foreach ( $routes[ $route ] as $handler ) {
		...
		if ( is_array( $methods ) && ! empty( $methods[ $method ] ) ) {
			return true;
		}

		if ( is_string( $methods ) && in_array( $method, array_map( 'trim', explode( ',', strtoupper( $methods ) ) ), true ) ) {
			return true;
		}
	}
...
tf_security_live_assert(
	in_array( tf_security_live_response_status( $admin_user_update_response ), array( 403, 404 ), true ),
	'Reported case failed: low-privilege REST request must not reach an administrator user update handler.'
);
...
if ( $vendor_user ) {
	...
	if ( tf_security_live_route_supports_method( '/tf/v1/get-google-access-token-url', 'GET' ) ) {
		tf_security_live_assert_status(
		...
		tf_security_live_assert_status(
			tf_security_live_rest_get( '/tf/v1/get-google-access-token-url' ),
			404,
			'Free-only REST runtime must not expose the Pro-owned Google access URL route.'
		);
	}
	...
	if ( tf_security_live_route_supports_method( '/tf/v1/reset-google-access-token', 'POST' ) ) {
		...
		tf_security_live_assert_status(
			tf_security_live_rest_post( '/tf/v1/reset-google-access-token' ),
			404,
			'Free-only REST runtime must not expose the Pro-owned Google reset route.'
		);
	}
}
