...
if ( ! class_exists( 'TF_Settings' ) ) {
	class TF_Settings {
		...
		public function tf_options() {
			...
			//Vendor Commission List submenu
			add_submenu_page(
				$this->option_id,
				__('commissions', 'tourfic'),
				__('commissions', 'tourfic'),
				'manage_options',
				'tf_vendor_commissions',
				array( $this,'tf_vendor_commissions_callback'),
			);
		 ...
		 * @author Jahid
		 */
		public function tf_vendor_list_callback(){
		?>
		<div class="tf-setting-dashboard">
			<!-- deshboard-top-section -->
			<div class="tf-setting-top-bar">
		...
		public function tf_vendor_commissions_callback(){
		?>
		<div class="tf-setting-dashboard">
			<!-- deshboard-top-section -->
			<div class="tf-setting-top-bar">
			...
			<div id="tf-report-loader">
				...
				<div class="wrap" style="margin-right: 20px;">
					<h1 class="wp-heading-inline"><?php _e( 'Commissions', 'tourfic' ); ?></h1>
					<?php 
						/**
						 *Vendor Class
						*/
						if ( file_exists( TF_INC_PATH . 'functions/class.tf_vendor_commission.php' ) ) {
							require_once TF_INC_PATH . 'functions/class.tf_vendor_commission.php';
						} else {
							tf_file_missing( TF_INC_PATH . 'functions/class.tf_vendor_commission.php' );
						}
						$tf_vendor_query = get_users( array( 'role__in' => array( 'tf_vendor' ) ) );
						$tf_vendors_list = new TFVENDORTable( $tf_vendor_query );
						$tf_vendors_list->prepare_items();
