...
if ( ! class_exists( 'TF_text' ) ) {
	class TF_tab extends TF_Fields {
		...
		public function render() { 
           ?>
		   	<div id="<?php if(isset($this->field['id'])){ echo esc_attr($this->field['id']); }  ?>" class="tf-tablist">
				<ul class="tf-nav-tabs">
					<?php if(isset($this->field['tabs']) && is_array($this->field['tabs'])): ?>
						<?php foreach($this->field['tabs'] as $key => $value):  ?>
							<li class="tf-tab-item <?php if($key == 0){echo "show";} ?>" data-tab-id="<?php if(isset($value['id'])){ echo esc_attr($value['id']); }  ?>"><?php echo $value['title'] ?></li> 
						<?php endforeach; ?>
					<?php endif; ?> 
				</ul>
