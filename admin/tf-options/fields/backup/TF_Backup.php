...
if ( ! class_exists( 'TF_Backup' ) ) {
    class TF_Backup extends TF_Fields {
        ...
        public function render() {
            ...
            echo '<textarea class="tf-exp-imp-field" cols="50" rows="15" name="tf_import_option" id="' . esc_attr( $this->field_name() ) . '"' . esc_attr($placeholder) . ' '. wp_kses_post($this->field_attributes()) .'> </textarea>';
            ...
            echo '<hr>';
            echo '<textarea cols="50" rows="15" class="tf-exp-imp-field"  name="tf_export_option" id="' . esc_attr( $this->field_name() ) . '"' . esc_attr($placeholder) . ' '. wp_kses_post($this->field_attributes()) .'disabled >' . wp_kses_post($current_settings) . '</textarea>';
