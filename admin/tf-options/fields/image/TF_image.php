...
if ( ! class_exists( 'TF_image' ) ) {
	class TF_image extends TF_Fields {
		...
		public function render() {
			...
			echo '<div class="tf-fieldset-media-preview ' . str_replace(array("[","]"),"_",esc_attr( $this->field_name() ) ) . '">';
			if(!empty($this->value)){
			...
			<input type="' . esc_attr( $type ) . '" name="' . esc_attr( $this->field_name() ) . '" id="' . esc_attr( $this->field_name() ) . '" placeholder="'. $this->field['placeholder'] .'" value="' . $this->value . '"  /><a href="#" tf-field-name="' . esc_attr( $this->field_name() ) . '" class="tf-media-upload button button-primary button-large">' . esc_html( "Upload","tourfic" ) . '</a></div>
